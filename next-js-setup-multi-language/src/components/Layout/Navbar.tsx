...
const Navbar: React.FC<NavbarProps> = ({ menuItems, brandName }) => {
  ...
  const locale = pathname.split('/')[1] || 'en'; // fallback to English
  ...
  return (
    <header className="bg-color-background text-color-foforeground w-full z-50 shadow">
      <div className="container mx-auto flex justify-between items-center p-4">
        ...
        <button onClick={toggleMobileMenu} className="md:hidden focus:outline-none" aria-label="Toggle menu">
                  ...
                  <>
                    ...
                    <ul className="absolute left-0 bg-gray-800 mt-2 rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 invisible group-hover:visible z-50 min-w-[150px]">
                      {item.submenu.map((subitem, idx) => (
                        <li key={idx}>
                         <Link key={subitem.path} href={`/${locale}${subitem.path === '/' ? '' : item.path}`}>
          {subitem.label}
        </Link>
                        </li>
                      ))}
                ...
                <li key={i} className="space-y-2">
                  ...
                  <ul className="ml-4 flex flex-col space-y-1">
                    {item.submenu.map((subitem, idx) => (
                      <li key={idx}>
                        <Link
                          href={`/${locale}${subitem.path === '/' ? '' : item.path}`}
                ...
                <li key={i}>
                  <Link
                   href={`/${locale}${item.path === '/' ? '' : item.path}`}
                    onClick={toggleMobileMenu}
                    className={
                      'block px-2 py-2 rounded hover:bg-gray-700 ' + (isActive(item.path) ? 'bg-gray-700 font-semibold' : '')
                    }
                  >
                    {item.label}
                  </Link>
      ...
      {mobileOpen && <div className="fixed inset-0 bg-black opacity-50 z-40 md:hidden" onClick={toggleMobileMenu}></div>}
    </header>
  );
};
